...
def _identifier(doc, auteur, titre, sujet):
    """
    Renseigne les proprietes du fichier : auteur, titre, sujet.

    Sans cela, Word affiche « python-docx » comme auteur et « generated by
    python-docx » en commentaire, ce que le premier clic droit sur le fichier
    revele. Un document rendu a un encadrant doit porter le nom de ceux qui
    l'ont ecrit, pas celui de la bibliotheque qui l'a assemble.
    """
    proprietes = doc.core_properties
    proprietes.author = auteur
    proprietes.last_modified_by = auteur
    proprietes.title = titre
    proprietes.subject = sujet
    proprietes.comments = ""
    proprietes.category = ""
    proprietes.keywords = ""
    proprietes.revision = 1

    # La date par defaut de python-docx est celle de son modele, en 2013.
    maintenant = datetime.now(timezone.utc).replace(microsecond=0)
    proprietes.created = maintenant
    proprietes.modified = maintenant


def nouveau_document(titre_pied, auteur="", titre="", sujet=""):
    doc = __import__("docx").Document()
    _identifier(doc, auteur, titre, sujet)
